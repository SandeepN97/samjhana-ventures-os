...
describe('EVManualEntryPage', () => {
  ...
  describe('charger selection', () => {
    ...
    it('renders Nepali labels and Devanagari numerals', async () => {
      renderWithProviders(<EVManualEntryPage />, { locale: 'ne' });
  ...
  describe('NEA rate visibility (business-sensitive: admins and managers only)', () => {
    const asRole = (role) => localStorage.setItem('user', JSON.stringify({ role, username: 'tester' }));

    it('shows the NEA rate banner to an admin and loads the rate', async () => {
      asRole('ADMIN');
      renderWithProviders(<EVManualEntryPage />);
      expect(await screen.findByText(/NEA Rate per Unit/)).toBeInTheDocument();
      expect(api.get).toHaveBeenCalledWith('/api/settings/nea_rate', expect.anything());
    });

    it('shows the NEA rate banner to a manager', async () => {
      asRole('MANAGER');
      renderWithProviders(<EVManualEntryPage />);
      expect(await screen.findByText(/NEA Rate per Unit/)).toBeInTheDocument();
    });

    it('hides the banner from staff and never requests the rate', async () => {
      asRole('STAFF');
      renderWithProviders(<EVManualEntryPage />);
      await waitFor(() => expect(api.get).toHaveBeenCalledWith('/api/ev-vehicles'));
    ...
    it('does not reveal a rate cached in this browser by an earlier admin session', async () => {
      asRole('STAFF');
